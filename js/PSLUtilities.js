...
/**
 * The modules holds the code of the original project at https://github.com/publicsuffix/list
 * and tries to modify the interface in order to let vue.js interact with it.
 * For more learning about public suffix list, see the following link:
 * 
...
export const PLSUtilities = {
    HOSTNAME_SLOT: 0,    // jshint ignore:line
    LABEL_INDICES_SLOT: 256,  //  -- / 256 (256/2 => 128 labels max)
    RULES_PTR_SLOT: 100,  // 100 / 400 (400-256=144 => 144>128)
    SUFFIX_NOT_FOUND_SLOT: 399,  //  -- / 399 (safe, see above)
    CHARDATA_PTR_SLOT: 101,  // 101 / 404
    EMPTY_STRING: '',
    SELFIE_MAGIC: 2,
    ...
    allocateBuffers: function (byteLength) {
        this.pslByteLength = byteLength + 3 & ~3;
        if (
            this.pslBuffer32 !== undefined &&
            this.pslBuffer32.byteLength >= this.pslByteLength
        ) {
            return;
        }
        if (this.wasmMemory !== undefined) {
            const newPageCount = this.pslByteLength + 0xFFFF >>> 16;
            const curPageCount = this.wasmMemory.buffer.byteLength >>> 16;
            const delta = newPageCount - curPageCount;
            if (delta > 0) {
                this.wasmMemory.grow(delta);
                this.pslBuffer32 = new Uint32Array(this.wasmMemory.buffer);
                this.pslBuffer8 = new Uint8Array(this.wasmMemory.buffer);
            }
        } else {
            this.pslBuffer8 = new Uint8Array(this.pslByteLength);
            this.pslBuffer32 = new Uint32Array(this.pslBuffer8.buffer);
        }
        this.hostnameArg = this.EMPTY_STRING;
        this.pslBuffer8[this.LABEL_INDICES_SLOT] = 0;
    },

    parse: function (text, toAscii) {
        // Use short property names for better minifying results
        const rootRule = {
            l: this.EMPTY_STRING,    // l => label
            f: 0,               // f => flags
...

        {
            ...
            treeData[this.RULES_PTR_SLOT] = iRootRule;

            const iCharData = treeData.length << 2;
            treeData[this.CHARDATA_PTR_SLOT] = iCharData;

            const byteLength = (treeData.length << 2) + (charData.length + 3 & ~3);
            this.allocateBuffers(byteLength);
            this.pslBuffer32.set(treeData);
            this.pslBuffer8.set(charData, treeData.length << 2);
        }

        this.fireChangedEvent();
    },

    setHostnameArg: function (hostname) {
        const buf = this.pslBuffer8;
        if (hostname === this.hostnameArg) { return buf[this.LABEL_INDICES_SLOT]; }
        if (hostname === null || hostname.length === 0) {
            this.hostnameArg = this.EMPTY_STRING;
            return (buf[this.LABEL_INDICES_SLOT] = 0);
        }
        hostname = hostname.toLowerCase();
        this.hostnameArg = hostname;
        let n = hostname.length;
        if (n > 255) { n = 255; }
        buf[this.LABEL_INDICES_SLOT] = n;
        let i = n;
        let j = this.LABEL_INDICES_SLOT + 1;
        while (i--) {
    ...
    getPublicSuffixPosJS: function () {
        const buf8 = this.pslBuffer8;
        const buf32 = this.pslBuffer32;
        const iCharData = buf32[this.CHARDATA_PTR_SLOT];

        let iNode = this.pslBuffer32[this.RULES_PTR_SLOT];
        let cursorPos = -1;
        let iLabel = this.LABEL_INDICES_SLOT;
        ...
        for (; ;) {
            ...
            if (iFound === 0) {
                ...
                buf8[this.SUFFIX_NOT_FOUND_SLOT] = 1;
                iFound = iCandidates;
            }
            iNode = iFound;
            ...
            if ((buf32[iNode + 0] & 0x00000200) !== 0) {
                if (iLabel > this.LABEL_INDICES_SLOT) {
                    return iLabel - 2;
                }
                break;
            }
    ...
    getPublicSuffix: function (hostname) {
        if (this.pslBuffer32 === undefined) { return this.EMPTY_STRING; }

        const hostnameLen = this.setHostnameArg(hostname);
        const buf8 = this.pslBuffer8;
        if (hostnameLen === 0 || buf8[0] === 0x2E /* '.' */) {
            return this.EMPTY_STRING;
        }

        const cursorPos = this.getPublicSuffixPosJS()//getPublicSuffixPos();
        if (cursorPos === -1) {
            return this.EMPTY_STRING;
        }

        const beg = buf8[cursorPos + 1];
        return beg === 0 ? this.hostnameArg : this.hostnameArg.slice(beg);
    },
    ...
    suffixInPSL: function (hostname) {
        if (this.pslBuffer32 === undefined) { return false; }
        ...
        buf8[this.SUFFIX_NOT_FOUND_SLOT] = 0;
        const cursorPos = this.getPublicSuffixPosJS()//getPublicSuffixPos();
        return cursorPos !== -1 &&
            buf8[cursorPos + 1] === 0 &&
            buf8[this.SUFFIX_NOT_FOUND_SLOT] !== 1;
    ...
    // toSelfie: function (encoder) {
    //     if (this.pslBuffer8 === undefined) { return ''; }
    //     if (encoder instanceof Object) {
    //         const bufferStr = encoder.encode(this.pslBuffer8.buffer, this.pslByteLength);
    //         return `${this.SELFIE_MAGIC}\t${bufferStr}`;
    //     }
    //     return {
    //         magic: this.SELFIE_MAGIC,
    //         buf32: Array.from(
    //             new Uint32Array(this.pslBuffer8.buffer, 0, this.pslByteLength >>> 2)
    //         ),
    //     };
    // },

    // fromSelfie: function (selfie, decoder) {
    //     let byteLength = 0;
    //     if (
    ...
    //         if (pos === -1 || selfie.slice(0, pos) !== `${this.SELFIE_MAGIC}`) {
    //             return false;
    //         }
    ...
    //         this.allocateBuffers(byteLength);
    //         decoder.decode(bufferStr, this.pslBuffer8.buffer);
    //     } else if (
    //         selfie instanceof Object &&
    //         selfie.magic === this.SELFIE_MAGIC &&
    //         Array.isArray(selfie.buf32)
    //     ) {
    //         byteLength = selfie.buf32.length << 2;
    //         this.allocateBuffers(byteLength);
    //         this.pslBuffer32.set(selfie.buf32);
    //     } else {
    //         return false;
    //     }
    ...
    //     return true;
    // },
    PLSData: `// This Source Code Form is subject to the terms of the Mozilla Public
    ...
    *.ye
    ...
    // List of new gTLDs imported from https://www.icann.org/resources/registries/gtlds/v2/gtlds.json on 2020-12-11T13:40:43Z
